...
class N_workchain(WorkChain):
    ...
    def define(cls, spec):
        super().define(spec)
        spec.input('temp_list', valid_type=List,
                help='temp list', required=False) 
        spec.input('N_list', valid_type=List,
                help='N list ', required=False) 
        spec.input('plot_dir', valid_type=Str,
                help='plot dir ', required=False) 
        spec.output('plot_name', valid_type=List)
        spec.outline(
    ...
    def plot_result(self):
        ...
        for i in N_list:
            ...
            data = cal_node_query(sub_workchain_pk,'temperature_output')[0][0].get_dict()
            ax.plot(np.array(data['temperature'])/T_C,np.array(data['magnetization'])/np.array(data['magnetization'][0]), label='N={}'.format(i))
        ax.legend()
        ax.set_xlabel('T/Tc')
        ax.set_ylabel('M/Mc')
        ...
        for i in N_list:
            ...
            self.report('sub_workchain {}'.format(sub_workchain_pk))
            data = cal_node_query(sub_workchain_pk,'temperature_output')[0][0].get_dict()
            ax.plot(np.array(data['temperature'])/T_C,data['energy'], label='N={}'.format(i))
        ax.legend()
        ax.set_xlabel('T/Tc')
