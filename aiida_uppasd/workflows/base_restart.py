...
class ASDBaseRestartWorkChain(BaseRestartWorkChain):
    """
    Base restart workchain: Workchain to run an UppASD simulation with automated error handling and restarts.
    ...
    def define(cls, spec):
        ...
        '''       
        #here I post two input methods from aiida base restart workchain, use it if needed
    ...
    def inputs_process(self):
        """ process all input data and store it into self.inputs node for auto restart operation"""
        ...
        self.ctx.inputs = {
            ...
            'metadata': {
                'options':{
                    'resources': {'num_machines': self.inputs.num_machines.value,'num_mpiprocs_per_machine':self.inputs.num_mpiprocs_per_machine.value},
        ...
        try:
            ...
            self.ctx.inputs['exchange'] = exchange
        except:
            pass
