...
class UppASDBaseParser(Parser):
    ...
    def dm_out_parser(self, dmdata_out_file):
        ...
        #because dgl nodes is start from 0 so we need to use atom site minus 1 to fit DGL
    ...
    def struct_out_parser(self, struct_out_file):
        ...
        try:
            ...
            atom_site_A, atom_site_B, rij_x, rij_y, rij_z, J_ij, rij = np.array(
    ...
    def parse(self, **kwargs):
        ...
        for name in retrieved_file_name_list:
            print(f'File name in output list: {name}')
            self.logger.info(f'File name in output list: {name}')
            if 'coord' in name:
                coord_filename = name
                # parse coord.xx.out
                self.logger.info(f"Parsing '{coord_filename}'")
                with output_folder.open(coord_filename, 'rb') as f:
                    coord = self.coord_file_parser(f)
                    output_coord = orm.ArrayData()
            ...
            if 'totenergy' in name:
                ...
                self.logger.info(f"Parsing '{totenergy_filename}'")
                with output_folder.open(totenergy_filename, 'rb') as f:
                    Iter_num_totenergy, Tot, Exc, Ani, DM, PD, BiqDM, BQ, Dip, Zeeman, LSF, Chir = self.total_energy_file_parser(
                        f)
                    output_totenergy = orm.ArrayData()
                    output_totenergy.set_array('Iter_num_totenergy',
                                               Iter_num_totenergy)
            ...
            if 'dmdata' in name:
                ...
                # parse dmdata.xx.out
                self.logger.info(f"Parsing '{dmdata_out_file}'")
            ...
            if 'cumulant' in name and not 'out' in name:
                cumulant_file = name
                self.logger.info(f"Parsing '{cumulant_file}'")
            ...
            if 'ams' in name[0:4]:
                ams_filename = name
                # parse qpoints.xx.out
                self.logger.info(f"Parsing '{ams_filename}'")
                            ...
                            with output_folder.open('posfile', 'r') as pfile:
                                ...
                                for idx, line in enumerate(lines):
                                    ...
                                    if len(line_data) > 0:
                                        positions = np.vstack(
                                            (positions,
                                             np.asarray(line_data[2:5])))
                                        numbers = np.append(
                                            numbers, np.asarray(line_data[1]))
