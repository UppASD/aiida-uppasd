...
vecarg_list = [
    'hfield', 'ip_hfield', 'qm_svec', 'qm_nvec', 'jvec', 'bc', 'ncell'
]
...
def inpsd_to_dict(f):
    ...
    for line in f:
        if (len(line) > 1):
            ...
            if (key == 'cell'):
                a1 = line.split()[1:4]
                a2 = f.readline().split()[0:3]
                a3 = f.readline().split()[0:3]
                data = np.asarray([
...
def dict_to_inpsd(inpdict, f=stdout):
    ...
    for key in inpdict.keys():
        if (key == 'cell'):
            ...
            for iline in range(nlines):
                print(' '.join([str(w) for w in inpdict[key][iline + 1]]),
                      file=f)
        elif (key in vecarg_list):
    ...
    try:
        with open(posfile, 'r') as pfile:
            ...
            for idx, line in enumerate(lines):
                ...
                if len(line_data) > 0:
                    positions = np.vstack(
                        (positions, np.asarray(line_data[2:5], dtype=float)))
                    numbers = np.append(numbers,
                                        np.asarray(line_data[1], dtype=float))
